...
\begin{document}
    ...
    図2のようにカメラで映し出された映像の指定範囲のピクセルに対してネガポジ反転処理を行うアプリを実装した. 
    このアプリでは, カメラから入力されたRGBの各輝度値 $I$ に対し, 出力値 $I'$ を $I' = 1.0 - I$ と定義することで, 色相と明度を反転させている. 


    \section{実\hspace{2zw}験}
    被験者は20から26歳の男性10名とした．被験者は図3のようにMeta Quest 3を装着した状態で，
    市販の同一製品のカップラーメンを，図4のようにネガポジ反転処理された状態と通常の視界で摂取した．
    ...
    \begin{itemize}
    \item \textbf{A群}：通常視界, ネガポジ反転視界の順で摂取
    \item \textbf{B群}：ネガポジ反転視界, 通常視界の順で摂取
    \end{itemize}
    \vspace{-10mm}
    ...
    「本来の味がわからなくなった」「色が不自然で脳が味を拒絶している感覚があった」といった意見が見られた.
    ...
    \begin{figure}[H]
    \begin{center}
      \includegraphics[width=0.7\linewidth]{fig/negapozi_noodle.jpg}
    ...
    \begin{figure}[h]
     \begin{center}
      \includegraphics[width=0.8\linewidth]{fig/negapozisystem.png}
